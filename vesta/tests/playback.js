...
if (window.setImmediate === undefined) {
    ...
    window.setImmediate = (func) => {
        ...
        window.postMessage("vestavnc immediate trigger:" + index, "*");
        return index;
    };
    ...
    window.addEventListener("message", (event) => {
        if ((typeof event.data !== "string") ||
            (event.data.indexOf("vestavnc immediate trigger:") !== 0)) {
            return;
        }

        const index = event.data.slice("vestavnc immediate trigger:".length);
